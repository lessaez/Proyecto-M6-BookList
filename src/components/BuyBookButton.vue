<script setup>
import { onMounted, onUnmounted, ref, watch } from 'vue'
import { hasPurchasedBook, purchaseBook } from '../utils/bookStore'

const props = defineProps({
  book: {
    type: Object,
    required: true
  },
  fullWidth: {
    type: Boolean,
    default: false
  }
})

const purchased = ref(false)

const syncPurchasedState = () => {
  purchased.value = hasPurchasedBook(props.book?.id)
}

const handlePurchase = () => {
  if (purchased.value) return

  purchaseBook(props.book)
  purchased.value = true
}

watch(
  () => props.book?.id,
  () => {
    syncPurchasedState()
  },
  { immediate: true }
)

onMounted(() => {
  window.addEventListener('compras-actualizadas', syncPurchasedState)
  window.addEventListener('storage', syncPurchasedState)
})

onUnmounted(() => {
  window.removeEventListener('compras-actualizadas', syncPurchasedState)
  window.removeEventListener('storage', syncPurchasedState)
})
</script>

<template>
  <button
    type="button"
    class="buy-button"
    :class="{
      'buy-button-full': fullWidth,
      'buy-button-done': purchased
    }"
    :aria-label="purchased ? `Compra registrada para ${book.titulo}` : `Comprar ${book.titulo}`"
    :title="purchased ? 'Compra registrada' : 'Comprar libro'"
    @click="handlePurchase"
  >
    <svg
      class="buy-icon"
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      aria-hidden="true"
      focusable="false"
    >
      <path
        fill="currentColor"
        d="M9.75 11.5a.75.75 0 0 1 .75.75v4a.75.75 0 0 1-1.5 0v-4a.75.75 0 0 1 .75-.75m5.25.75a.75.75 0 0 0-1.5 0v4a.75.75 0 0 0 1.5 0z"
      />
      <path
        fill="currentColor"
        d="M10.68 2.136a.75.75 0 0 1 .184 1.044L8.365 6.75h7.27l-2.5-3.57a.75.75 0 0 1 1.23-.86l3.1 4.43h2.285A2.25 2.25 0 0 1 22 9v.785a2.25 2.25 0 0 1-.833 1.748l-1.163.943l-.803 6.308a2.25 2.25 0 0 1-2.232 1.966H7.031a2.25 2.25 0 0 1-2.232-1.966l-.804-6.308l-1.162-.943A2.25 2.25 0 0 1 2 9.785V9a2.25 2.25 0 0 1 2.25-2.25h2.284l3.102-4.43a.75.75 0 0 1 1.044-.184m9.07 6.114H4.25A.75.75 0 0 0 3.5 9v.785c0 .226.102.44.278.583L5.174 11.5a.75.75 0 0 1 .271.488l.842 6.607a.75.75 0 0 0 .744.655h9.938a.75.75 0 0 0 .744-.655l.841-6.607a.75.75 0 0 1 .272-.488l1.396-1.132a.75.75 0 0 0 .278-.583V9a.75.75 0 0 0-.75-.75"
      />
    </svg>
    <span class="buy-label">{{ purchased ? 'Comprado' : 'Comprar' }}</span>
  </button>
</template>

<style scoped>
.buy-button {
  display: inline-flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
  gap: 5px;
  min-height: 54px;
  min-width: 0;
  padding: 10px 8px;
  border: none;
  border-radius: 12px;
  background: linear-gradient(135deg, #ff9f1c, #ffbf69);
  color: #2f2a33;
  font-weight: 700;
  font-size: 0.95rem;
  line-height: 1.15;
  text-align: center;
  cursor: pointer;
  transition: transform 0.2s ease, box-shadow 0.2s ease, opacity 0.2s ease;
  box-shadow: 0 8px 18px rgba(255, 159, 28, 0.24);
}

.buy-button:hover {
  transform: translateY(-1px);
}

.buy-button-full {
  width: 100%;
}

.buy-button-done {
  background: linear-gradient(135deg, #42b983, #7ad7aa);
  color: #ffffff;
  box-shadow: 0 8px 18px rgba(66, 185, 131, 0.22);
}

.buy-icon {
  width: 18px;
  height: 18px;
  flex-shrink: 0;
}

.buy-label {
  min-width: 0;
  white-space: normal;
  overflow-wrap: break-word;
  text-wrap: balance;
}
</style>
